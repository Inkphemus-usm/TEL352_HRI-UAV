...
Requisitos:
  pip install pymavlink opencv-python mediapipe numpy scikit-learn tensorflow psutil
...
import argparse
from pymavlink import mavutil
import psutil # NUEVA IMPORTACIÓN PARA RECURSOS
...
# --- 4. CARGA DE MODELO Y MÉTRICAS ESTÁTICAS ---
...
try:
    ...
    # --- MÉTRICAS ESTÁTICAS Y TAMAÑO ---
    model_size_mb = os.path.getsize(MODEL_PATH) / (1024 * 1024)
    logging.info('--- ESPECIFICACIONES DEL MODELO ---')
    logging.info('Tamaño del modelo en disco: %.2f MB', model_size_mb)
    # Remplaza los valores de precisión con los resultados de tu set de prueba:
    logging.info('Precisión (Accuracy) reportada en Test Set: 94.5 %%')
    logging.info('F1-score reportado en Test Set: 0.93') 
    logging.info('------------------------------------')
...
def run():
    ...
    inference_times = []
    max_inference_samples = 100 # Número de muestras para promediar
    # ---------------------------
    ...
    with mp_pose.Pose(model_complexity=1, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        try:
            ...
            if inference_times:
                ...
                logging.info('\n--- RESULTADOS DE MÉTRICAS EN TIEMPO DE EJECUCIÓN ---')
                logging.info('Latencia Promedio de Inferencia: %.2f ms (sobre %d muestras)', avg_latency, len(inference_times))
            ...
            try:
                ...
                # Uso de CPU (se recomienda monitoreo externo para un valor representativo del proceso completo)
                cpu_percent = process.cpu_percent(interval=None) 
                
                logging.info('--- CONSUMO DE RECURSOS (Al Finalizar) ---')
                logging.info('Memoria RAM (RSS) utilizada: %.2f MB', ram_mb)
                logging.info('Uso de CPU del proceso (último instante): %.2f %%', cpu_percent)
                logging.info('--------------------------------------------')
                
            except Exception as e:
                logging.warning('Error al medir recursos con psutil: %s', e)
